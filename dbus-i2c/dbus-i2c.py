...
def update_adc():
    ...
    for channel in [0, 1, 7]:
        if not os.path.exists('/sys/bus/iio/devices/iio:device0'):
            ...
            with open('/sys/bus/iio/devices/iio:device0/in_voltage' + str(channel) + '_raw', 'r') as fd:
                value = 0
...
def handle_changed_setting(setting, oldvalue, newvalue):
    ...
    [path, obj] = settingObjects[setting]
    obj[path] = newvalue
    return True
...
parser.add_argument("-n", "--name", help="the D-Bus service you want me to claim", type=str,
                    default="com.victronenergy.i2c")
